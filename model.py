...
class DocREModel(nn.Module):
    ...
    def forward(self,
        ...
        #GCN层数为1
        # nodes_re = gcn_nodes[-1]
        # new_nodes_re = new_gcn_nodes[-1]
        ...
        relation = torch.cat(relation, dim=0)       #rrs，把所有batch的relation拼接起来
        ...
        # GCN层数为1时，取头尾实体表示
        #new_entity_ht = torch.cat([new_entity_h, new_entity_t], dim=-1)

        #对实体对表示进行注意力加权
        # entity_ht = self.MIP_Linear3(entity_ht)
        # e_tw = self.MIP_Linear3(e_tw)
        # new_entity_ht = self.MIP_Linear3(new_entity_ht)
        # combined_tensor = torch.cat([entity_ht, e_tw, sc_feature_e, new_entity_ht], dim=-1)
        # scores = self.linear(combined_tensor)
        # weights = self.softmax(scores)
        # relation_rep = torch.sum(weights * combined_tensor, dim=-1)
