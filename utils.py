...
def collate_fn(batch):
    ...
    sub_adjacency = [f["sub_adjacency"] for f in batch]
    nodes_info = [f["nodes_info"] for f in batch]           
    nodes_info = [torch.tensor(item, dtype=torch.long) for item in nodes_info]
    sub_nodes = [f["sub_nodes"] for f in batch]
    sub_nodes = [torch.tensor(item, dtype=torch.long) for item in sub_nodes]
    input_ids = torch.tensor(input_ids, dtype=torch.long)
    input_mask = torch.tensor(input_mask, dtype=torch.float)
    # output = (input_ids, input_mask, labels, entity_pos, hts)
    output = (input_ids, input_mask, labels, entity_pos, hts, adjacency, link_pos, nodes_info, dists, sub_nodes, sub_adjacency)
    return output
